...
public class TrainGear extends Controller {
	...
	private QLearning ql = new QLearning("qtable_gear.csv");
	...
	private String getEstado(SensorModel sensors) { // 72 estados
		int rpmBin;
	...
	private double getRecompensa(SensorModel sensors, double lastSpeed) {
		double recompensa = 0.0;

		double aceleracion = sensors.getSpeed() - lastSpeed;
		recompensa += aceleracion * 0.05;

		if (sensors.getRPM() > 7000)
			recompensa -= 2;
		if (sensors.getRPM() < 2000)
			recompensa -= 1;
	...
	private int getGear(SensorModel sensors) {
		...
		int accion = ql.elegirAccionTrain(estado);

		if (lastEstado != null) {
			double recompensa = getRecompensa(sensors, lastSpeed);
